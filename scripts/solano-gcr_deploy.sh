...
if [ -n "$DEPLOY_GCR" ] && [[ "true" == "$DEPLOY_GCR" ]]; then
  # Deploy to unique tag for historical record
  ...
  # Deploy to $GCR_DOCKER_TAG for pull on next ci build
  sudo docker tag -f $GCR_DOCKER_USER/$GCR_DOCKER_APP $GCR_DOMAIN/$GCR_PROJECT_ID/$GCR_DOCKER_APP:$GCR_DOCKER_TAG 
